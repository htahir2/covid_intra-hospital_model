...
class IntraHospModel(Model):
    ...
    results_dir = '/PATH_TO_THE_FOLDER_WHERE run.py EXISTS/'
                    ...
                    for val in hcw.contact_tracing[:]:
                        ...
                        for hcw1 in hcws:
                            if hcw1.unique_id == val[0]: ## check if id of hcws match with the individuals contacts
                                ...
                                if (self.schedule.time - val[1])<params.testing_day_simtime and hcw1.hcw_on_ppe_cont_tracing == 0 and (hcw1.state == State.SUSCEPTIBLE or hcw1.state == State.EXPOSED or hcw1.state == State.ASYMPTOMATIC or hcw1.state == State.RECOVERED):## means testing time not yet arrived, put hcw contacts on ppe
                        ...
                        for pat in self.schedule.agents_by_type[Patient]:
                            if pat.unique_id == val[0] and val[0] >= 5000: ## check if id of patients match with the individuals contacts
                                ...
                                if (self.schedule.time - val[1])<params.testing_day_simtime and int(pat.ward) >= 9 and pat.patient_isolated == 0 and (pat.state == State.SUSCEPTIBLE or pat.state == State.EXPOSED or pat.state == State.ASYMPTOMATIC or pat.state == State.RECOVERED):## means testing time not yet arrived, put patient contacts on isolation, if we isolated patients in the covid wards also, it will reduce the tranmission even further. there hcw wear ppe + reduction due to patient in isolation.
